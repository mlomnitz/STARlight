...
//float const width = 0.14;
//v2
float const length = 0.005;
float const width = 0.1;
void setGraphicsStyle()
{
    ...
    //Lomnitz:: Used to be 2.0
    //gStyle->SetTitleOffset(20.0,"Y");
    //Lomnitz:: new
...
#ifdef _QM17_
  if(!square) cv = new TCanvas(Form("cv%i",nCvs),Form("cv%i",nCvs),2*450,2*300);
#else
...
void drawErrors(TGraphErrors* gr)
{
  ...
  for(int ii = 0; ii< gr->GetN(); ++ii){
    ...
    //la->Draw("same");
    if(y1>-0.05) la->Draw("same");
    
    TLine *lb = new TLine(x2, y1, x2, y1+length);
    lb->SetLineColor(col);
    lb->SetLineWidth(lineWidth);
    //lb->Draw("same");
    if(y1>-0.05) lb->Draw("same");
    
    TLine *lc = new TLine(x1, y2, x1, y2-length);
    lc->SetLineColor(col);
    lc->SetLineWidth(lineWidth);
    lc->Draw("same");

    TLine *ld = new TLine(x2, y2, x2, y2-length);
    ld->SetLineColor(col);
    ld->SetLineWidth(lineWidth);
    ld->Draw("same");
    
    TLine *le = new TLine(x1, y1, x2, y1);
    le->SetLineWidth(lineWidth);
    le->SetLineColor(col);
    //le->Draw("same");
    if(y1>-0.05) le->Draw("same");

    TLine *lf = new TLine(x1, y2, x2, y2);
    lf->SetLineColor(col);
    lf->SetLineWidth(lineWidth);
    lf->Draw("same");
  }   
}
void drawErrors_mTplot(TGraphErrors* gr)
{
  int lineWidth = 1;
  Color_t col = gr->GetMarkerColor();

  for(int ii = 0; ii< gr->GetN(); ++ii){
    double x1 = gr->GetX()[ii]-width/2.;
    double x2 = gr->GetX()[ii]+width/2.;
    double y1 = gr->GetY()[ii]-gr->GetEY()[ii];
    double y2 = gr->GetY()[ii]+gr->GetEY()[ii];
    
    TLine *la = new TLine(x1, y1, x1, y1+length/2.);
    la->SetLineColor(col);
    la->SetLineWidth(lineWidth);
    //la->Draw("same");
    if(y1>-0.03) la->Draw("same");
    
    TLine *lb = new TLine(x2, y1, x2, y1+length/2.);
    lb->SetLineColor(col);
    lb->SetLineWidth(lineWidth);
    //lb->Draw("same");
    if(y1>-0.03) lb->Draw("same");
    
    TLine *lc = new TLine(x1, y2, x1, y2-length/2.);
    lc->SetLineColor(col);
    lc->SetLineWidth(lineWidth);
    lc->Draw("same");

    TLine *ld = new TLine(x2, y2, x2, y2-length/2.);
    ld->SetLineColor(col);
    ld->SetLineWidth(lineWidth);
    ld->Draw("same");
